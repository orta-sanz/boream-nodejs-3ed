...
class PetRouter {
    ...
    static async getAll(ctx) {
        ...
        ctx.body = await petService.getAll(ctx.query.page && +ctx.query.page > 0 ? ctx.query.page : undefined, ctx.query.name, ctx.query.sort);
    }
    ...
    static async updateComplete(ctx){
        logger.info(`Update complete pet with id ${ctx.params.id} with body: `, ctx.request.body);
        ctx.body = await petService.updateComplete(ctx.params.id, ctx.request.body);
    }

    static async updatePartial(ctx) {
        logger.info(`Update partial pet with id ${ctx.params.id} with body: `, ctx.request.body);
        ctx.body = await petService.updatePartial(ctx.params.id, ctx.request.body);
    }

    static async delete(ctx){
        logger.info(`Delete pet with id ${ctx.params.id}`);
...
const checkExist = async (ctx, next) => {
    logger.debug('Checking if exist pet with id', ctx.params.id);
